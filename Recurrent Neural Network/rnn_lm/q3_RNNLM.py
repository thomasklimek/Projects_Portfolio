...
class RNNLM_Model(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(tf.int32, shape=(self.config.batch_size, self.config.num_steps))
    self.labels_placeholder = tf.placeholder(tf.int32, shape=(self.config.batch_size, self.config.num_steps))
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("RNNLM", reuse=tf.AUTO_REUSE):
        ...
        inputs = [tf.squeeze(i) for i in tf.split(embeds, self.config.num_steps, 1)]
        inputs = tf.reshape(tf.convert_to_tensor(inputs), (self.config.num_steps, self.config.batch_size, self.config.embed_size))
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("RNNLM"):
      ...
      outputs = [tf.matmul(tf.nn.dropout(i, self.dropout_placeholder), U) + b_2 for i in rnn_outputs]
      ### END YOUR CODE
    return outputs
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    self.labels_placeholder = tf.convert_to_tensor(self.labels_placeholder)
    w = tf.constant(1.0, shape= (self.config.batch_size, self.config.num_steps))
    loss = sequence_loss(output, self.labels_placeholder, w)
    ### END YOUR CODE
    return loss
  ...
  def __init__(self, config):
    ...
    #self.predictions = tf.map_fn(lambda o: tf.nn.softmax(tf.cast(o, 'float64')), self.outputs)
    self.predictions = [tf.nn.softmax(tf.cast(o, 'float64')) for o in self.outputs]
    #[tf.nn.softmax(tf.cast(o, 'float64')) for o in self.outputs]
  ...
  def add_model(self, inputs):
    ...
    #inputs = np.reshape(inputs, (inputs.shape[1],inputs.shape[0],inputs.shape[2]))

    with tf.variable_scope('RNNLM', reuse=tf.AUTO_REUSE):
      #inputs = tf.transpose(tf.stack(inputs))
      ...
      outputs, self.final_state = tf.nn.dynamic_rnn(cell, inputs, initial_state = self.initial_state, dtype=tf.float32)
      """
    #end try
      #outputs = np.zeros((inputs.shape[0], inputs.shape[1], self.config.hidden_size))
      for step in range(self.config.num_steps):
        curr_state = tf.nn.sigmoid(tf.matmul(curr_state, H) + tf.matmul(inputs[step], I) + b_1)
        outputs.append(curr_state)

      self.final_state = curr_state
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in range(stop_length):
    ### YOUR CODE HERE
    t = np.array([tokens[i]]).reshape((-1,1))
    state, y_pred = session.run([model.final_state, model.predictions[-1]],
                    feed_dict={model.input_placeholder: t, model.initial_state: state,
...
def test_RNNLM():
  ...
  with tf.Session() as session:
    ...
    """
    import pickle
    mpickle_out = open("model.pickle", "wb")
    gpickle_out = open("gen_config.pickle", "wb")
    pickle.dump(model, mpickle_out)
    pickle.dump(gen_config, gpickle_out)
    mpickle_out.close()
    gpickle_out.close()
    #serialize for debug
      ...
      gen_config = pickle.load(open("gen_config.pickle", "rb"))
      """
