...
void SceneParser::flatten(){
   flattenSceneNode(getRootNode(), glm::mat4(1.0f));
}

// depth-first scene flattening
void SceneParser::flattenSceneNode(SceneNode* node, glm::mat4 currentMatrix){
   bool changed = false;

   // rotate, scale, and translate
   for (int i = 0; i < node->transformations.size(); i++){
      if (node->transformations[i]->type == TRANSFORMATION_ROTATE){
         glm::vec3 rotate = node->transformations[i]->rotate;
         if (glm::length(rotate) > EPSILON){
            glm::mat4 R = glm::rotate(glm::mat4(1.0f), node->transformations[i]->angle, rotate);
         ...
         glm::vec3 scale = node->transformations[i]->scale; 
      ...
      else if (node->transformations[i]->type == TRANSFORMATION_TRANSLATE){
         glm::vec3 translate = node->transformations[i]->translate;  
   ...
   // add to flattenedTree
   for (int i = 0; i < node->primitives.size(); i++){
      ScenePrimitive *primitive = node->primitives[i];
      struct RenderedPrimitive flattenedNode;

      ppm* texMap =new ppm(primitive->material.textureMap->filename);
      primitive->material.textureMap->data = texMap;
      cout << texMap->getWidth() << " width " << endl;


      flattenedNode.primitive = primitive;
      flattenedNode.modelView = currentMatrix;
      //flattenedNode.textureMap = primitive->material->textureMap->filename

     // cout << primitive->material.textureMap->filename << endl;

      //ppm* texMap =new ppm(primitive->material.textureMap->filename);
      //primitive->material.textureMap->data = texMap;
      cout << texMap->getWidth() << " width " << endl;
      //flattenedNode.textureMap = texMap;

      flattenedNode.changedSinceLastNode = changed;
      flattenedTree.push_back(flattenedNode);
   }

   // then flatten children (this creates depth first order in the arraya)
   for (int i = 0; i < node->children.size(); i++){
      flattenSceneNode(node->children[i], currentMatrix);
   }
}

std::vector<struct RenderedPrimitive> SceneParser::getFlattenedTree(){
