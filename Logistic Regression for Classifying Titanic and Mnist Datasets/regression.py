...
class MyLogisticReg:
	...
	def sto_loss_function(self, w):
		...
		idx = np.random.randint(0, len(self.x)-10)
	...
	def fit(self, X, y):
		...
		while(diff > 1e-4):
			...
			for i in range(self.steps):	
				t += 1
				# variant step size
				w = w_prev - ((1/(1000*(1+t))) * gradient(w))
...
def main():
	X, Y = read_data('mnist-train.csv')
	
	print(X)
	print(Y)
	(X_train, Y_train, X_test, Y_test ) = split_data(X, Y, 0.8)
	options = None
	model = MyLogisticReg(options)
